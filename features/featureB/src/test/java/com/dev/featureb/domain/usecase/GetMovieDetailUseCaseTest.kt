...
class GetMovieDetailUseCaseTest {
    ...
    @Test
    fun `invoke should return MovieResultDetail from repository has success`() = runBlocking {
        // Given
        val expectedResult = MockResponse.fakeMovieResultDetailResponse().toDomain()
        every { repository.getMovieDetail(MOVIE_ID) } returns flowOf(expectedResult)

        // When
        val result = useCase(MOVIE_ID)

        // Then
        verify(exactly = ONCE) { repository.getMovieDetail(MOVIE_ID) }
    ...
    fun `invoke should return error from repository has error`() {
        ...
        every { repository.getMovieDetail(MOVIE_ID) } returns flow { throw error }
